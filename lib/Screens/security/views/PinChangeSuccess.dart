...
class PinChangeSuccess extends StatefulWidget {
  const PinChangeSuccess({Key? key}) : super(key: key);

  @override
  _PinChangeSuccessState createState() => _PinChangeSuccessState();
}

class _PinChangeSuccessState extends State<PinChangeSuccess> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Listen to checkmark animation completion and navigate back
    _checkmarkController.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        Future.delayed(const Duration(milliseconds: 800), () {
          if (mounted) {
            context.pop(); // Goes back to security page
          }
        });
      }
    });
